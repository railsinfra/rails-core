//! Append audit events after accounts mutations (RAI-14). Failures never change the HTTP outcome.
...
pub async fn emit_accounts_mutation(
    ...
    let Some(client) = audit_client.as_ref() else {
        return;
    };
    ...
    match res {
        ...
        Err(e) => {
            ...
            tracing::error!(
                ...
                grpc_status = %e,
                "audit append failed after accounts mutation"
            );
            sentry::add_breadcrumb(sentry::Breadcrumb {
                category: Some("audit".into()),
                message: Some(format!(
                    "audit.append_failed action={action} correlation_id={cid} source_service=accounts grpc={e}"
                )),
                level: sentry::Level::Error,
                ..Default::default()
            });
            sentry::configure_scope(|scope| {
                scope.set_tag("source_service", "accounts");
                scope.set_tag("audit.action", action);
...
mod tests {
    ...
    async fn emit_skips_when_no_client() {
        let client: Option<AuditServiceClient<Channel>> = None;
        let mut headers = HeaderMap::new();
        headers.insert("x-environment", "sandbox".parse().unwrap());
